<template>
  <div class="app-container calendar-list-container">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="4">
            <el-form-item label="用户验证状态">
              ...
              >
                 <el-option
                   v-for="item in getEnumsMap('u_auth_status')"
                   :key="item.value"
                   :label="item.text"
                   :value="item.value">
                 </el-option>
            ...
            <el-form-item label="用户状态">
              ...
              >
                 <el-option
                   v-for="item in getEnumsMap('u_status')"
                   :key="item.value"
                   :label="item.text"
                   :value="item.value">
                 </el-option>
    ...
    >
      ...
      <!-- <el-table-column label="用户状态" :filters="getEnumsMap('u_status')" column-key="u_status" prop="u_status"> -->
      <el-table-column label="用户状态">
        <template slot-scope="scope">
          <el-tag :type="scope.row.u_status | statusFilter('u_status')">{{scope.row.u_status | getEnumLabel('u_status', $root)}}</el-tag>
        </template>
      </el-table-column>
      <!-- <el-table-column label="用户验证状态" :filters="getEnumsMap('u_auth_status')" column-key="u_auth_status" prop="u_auth_status"> -->
      ...
      <el-table-column align="center" label="操作" class-name="small-padding">
        <template slot-scope="scope">
          <router-link :to="{ name: 'UserUpdate', params: {u_id : scope.row.u_id} }">
...
export default {
  ...
  methods: {
    ...
    getEnumsMap(name) {
      const enums = this.$store.getters.enums
      if(enums[name]){
        const map = []
